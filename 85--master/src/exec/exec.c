...
/*   Updated: 2023/04/23 06:49:28 by dojannin         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int ft_exec(t_list *str) 
{
	...
	res = 0;
    //afflist(str, "EXEC LIST\n");
	//afftoken(g_data.token, "EXEC TOKEN");
	printf("str->value = %s\n", (char *)str->content);
	if (det_builtin() == 1)
		return (1);
	else if (det_hd() == -1)
		return (0);
	else if (ft_redir_pipe_detect(str) == 1)
		res = ft_redir_pipe(str);
	else if (ft_redir_detec_left(str) >= 1)
		res = ft_redir_left(str, NULL);
    else if (ft_redir_detec_right(str) >= 1)
	{
        res = ft_redir_right(str, NULL);
		if (res == 258)
			return (res);
	}
	else if (ft_pipe_detec(str) == 0)
    {
